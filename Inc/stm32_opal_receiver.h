...
#define OPAL_OVERSAMPLING_FACTOR 2
#define OPAL_SAMPLING_FREQ (OPAL_TRANSMISSION_FREQ * OPAL_OVERSAMPLING_FACTOR)

#define OPAL_ADC_BUFFER_SIZE (2 * OPAL_FRAME_BUFFER_SIZE * OPAL_OVERSAMPLING_FACTOR) // Size (in Symbol/Sample) of ADC buffer for DMA (Almost 2x the preamble size to ensure detection)
...
typedef struct {
    ...
    volatile size_t                 Buffer_offset;  /* !< Offset of the current ready half DMA buffer (Volatile for ITR access) */
    volatile uint16_t               DTC_idx;        /* !< Index of detected preamble in current buffer used as the phase reference (Volatile for ITR access) */
